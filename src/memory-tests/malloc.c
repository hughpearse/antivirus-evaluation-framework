/* Title: Test memory allocation to check for RAM virtualization
   Compile: wine gcc.exe memory-tests/malloc.c -o bin/malloc.exe
*/
...
{
	char * memdmp = NULL;
	memdmp = (char *) malloc(10);
	if(memdmp!=NULL)
        {
                memset(memdmp,'A', 10);
		uint32_t x = crc32str(memdmp);
		//crc32str('AAA...') = 0e4ff8a2
		//printf("crc32: %08x\n", endian(x));
                free(memdmp);

		if(endian(x) == 0x0e4ff8a2){
			printf("crc32 is correct, memory was allocated!\n");
