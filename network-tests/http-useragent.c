/* Title: Write EICAR string to user-agent string of HTTP request
   Compile: wine gcc.exe network-tests/http-useragent.c -lws2_32 -o bin/http-useragent.exe
*/
...
char USERAGENT[72] = {0}; //zero the memory first
...
char *build_get_query(char *host, char *page)
{
	...
	char *tpl = "GET /%s HTTP/1.0\r\nHost: %s\r\nUser-Agent: %s\r\n\r\n";
        xor_encrypt_decrypt(key, ciphertext, USERAGENT);
	if(getpage[0] == '/'){
		getpage = getpage + 1;
